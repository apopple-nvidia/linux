...
impl NpdeStruct {
    fn new(pdev: &pci::Device, data: &[u8]) -> Result<Self> {
        ...
        if subimage_len == 0 {
            dev_err!(pdev.as_ref(), "Invalid subimage length: 0\n");
            return Err(EINVAL);
        }
    ...
    fn find_in_data(
        ...
        NpdeStruct::new(pdev, &data[npde_start..])
            .inspect_err(|e| {
                dev_dbg!(pdev.as_ref(), "Error creating NpdeStruct: {:?}\n", e);
            })
            .ok()
    }
}
...
bios_image! {
    ...
    FwSecPartial FwSecBiosBuilder, // FWSEC (Firmware Security)
}
...
impl TryFrom<BiosImageBase> for BiosImage {
    ...
    fn try_from(base: BiosImageBase) -> Result<Self> {
        match base.pcir.code_type {
            ...
            0xE0 => Ok(BiosImage::FwSecPartial(FwSecBiosBuilder {
                base,
