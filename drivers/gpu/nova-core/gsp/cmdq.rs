...
impl GspCmdq {
    ...
    pub(crate) fn alloc_gsp_queue_command<'a>(
        ...
        let msg_header = GspMsgHeader::from_mut_bytes(msg_header_slice).ok_or(EINVAL)?;
    ...
    pub(crate) fn receive_msg_from_gsp<'a>(self: &'a mut Self) -> Result<GspQueueMessage<'a>> {
        ...
        let (slice_1, slice_2) = if rpc_header.length + HEADER_SIZE < remaining {
            (
                &msg_slice[(HEADER_SIZE as usize)..(HEADER_SIZE + rpc_header.length) as usize],
                None,
            )
        } else {
            ...
            let slice_2 = unsafe {
                core::slice::from_raw_parts(
                    ptr as *const u8,
                    rpc_header.length as usize - slice_1.len(),
                )
            };
